...
| `flightId` | path | 航班的 ID | 是 | string (uuid) |
...
| `result` | UserFlight | 来自飞行日志的一条航班记录。 |
...
| `aircraftId` | string (uuid) | 所使用飞机的 ID |
| `liveryId` | string (uuid) | 所使用涂装的 ID。**目前仅在 Casual 服务器上支持** |
| `callsign` | string | 用户在此航班中的呼号 |
| `server` | string | 执行该航班时所在服务器的名称 |
| `dayTime` | float | 白天飞行时间，单位为分钟 |
| `nightTime` | float | 夜间飞行时间，单位为分钟 |
| `totalTime` | float | 航班总飞行时间，单位为分钟 |
| `landingCount` | integer | 航班中完成的着陆次数 |
| `originAirport` | string | 出发机场的 ICAO 代码。可以为 null |
| `destinationAirport` | string | 到达机场的 ICAO 代码。可以为 null |
| `xp` | integer | 航班中获得的 XP 数量 |
| `worldType` | integer | 执行该航班时所在服务器的类型。_枚举：_ `"Solo = 0"`, `"Casual = 1"`, `"Training = 2"`, `"Expert = 3"`, `"Private = 4"` |
| `violations` | [Violation] | 用户在此航班中收到的违规记录数组。 |
...
| 名称          | 类型             | 描述 |
| ------------- | ------------- | ----------- |
| `issuedBy`    | Issuer        | 关于发出违规记录的人员的信息 |
| `level`       | integer       | 违规的严重等级 |
| `type`        | string        | 所犯违规的类型 |
...
| `callsign`       | string | 发出者的呼号 |
| `discourseUser`  | DiscourseUser | 关于发出者的 Discourse 账户的信息（如果可用） |
...
| `virtualOrganization`| string | 与该用户关联的虚拟组织（如果有） |
| `avatarTemplate`     | string | 该用户在 Discourse 上头像的 URL 模板 |